...
# Get a list of jobs so we know the clusters ids
jobList=$(curl -v -s https://$workspaceUrl/api/2.0/jobs/list \
            -H "Authorization:Bearer $accessToken" \
            -H "X-Databricks-Azure-SP-Management-Token: $managementToken" \
            -H "X-Databricks-Azure-Workspace-Resource-Id: $resourceId" \
            -H "Content-Type: application/json")
...
find . -type f -name "*" -print0 | while IFS= read -r -d '' file; do
    ...
    # Print jobList before processing it with jq
    ...
    then
        ...
        # Print clusterList before processing it with jq
        echo "clusterList: $clusterList"
...
######################################################################################
# Run jobs right after deployment, further set a schedule for the jobs
######################################################################################

jobId=$(echo $jobList | jq -r ".jobs[] | select(.settings.name == \"$jobName\") | .job_id")
runName="Run after deployment"
...
curl -X POST https://$workspaceUrl/api/2.0/jobs/runs/submit \
    -H "Authorization:Bearer $accessToken" \
    -H "X-Databricks-Azure-SP-Management-Token: $managementToken" \
    -H "X-Databricks-Azure-Workspace-Resource-Id: $resourceId" \
    -H "Content-Type: application/json" \
